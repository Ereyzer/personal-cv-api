import Joi from 'joi';
import { isFacebookLink, isGithubLink, isInstagramLink, isLinkedinLink, isPhoneLink, } from "./linkValidators.js";
...
const simleInfoFields = simpleFieldsSchema['$_terms'].keys.reduce((prev, { key }) => {
    if (!key) {
        return [...prev];
    }
    else {
...
const langFialds = langFieldSchema['$_terms'].keys.reduce((prev, { key }) => {
    if (!key) {
        return [...prev];
    }
    else {
